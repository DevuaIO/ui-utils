...
function filterInternal<T>(input: T, opts: CleanOptions): T {
  ...
  if (Array.isArray(input)) {
    return input
      .map((item) => {
        const processed = typeof item === "object" && item !== null ? filterInternal(item, opts) : item;
  ...
  if (typeof input === "object") {
    ...
    for (const [key, value] of Object.entries(input as object)) {
      const processed = typeof value === "object" && value !== null ? filterInternal(value, opts) : value;
